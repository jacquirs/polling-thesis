...
def load_and_prepare(filepath: str, state: str, election_date: str = '2024-11-05', days_before: int = None) -> pd.DataFrame:
    ...
    n_questions = len(df)
    print(f"{state} polls loaded: {n_questions} questions from {n_polls} polls")
...
def aggregate_by_day(df: pd.DataFrame) -> pd.DataFrame:
    """
    collapse all polls with the same end_date into a single inverse-variance weighted "meta-poll" before passing the data to the kalman filter

    this makes the one-observation-per-time-point assumption of the green et al (1999) state-space model literally true
    without this step, the kalman filter in kalman_national_harrisonly_analysis.py processes 10-15 polls per day sequentially, which is internally consistent (days_elapsed=0 means no opinion drift) but conflates variation due to house effects with variation in true opinion

    why inverse-variance weighting?: if we have k polls on day t with margins m_1...m_k and sampling variances v_1...v_k, the optimal (minimum variance) linear combination is
    weights: w_i = 1/v_i
    combined margin: m* = sum(w_i * m_i) / sum(w_i)
    combined variance: v* = 1 / sum(w_i)

    polls with larger samples (smaller variance) get more weight in the daily average
    the combined variance is always smaller than any individual poll's variance, because information adds across polls
    this is equivalent to treating all same-day polls as draws from the same underlying population on the same day

    what is lost:
    combining all polls on a day into one number destroys pollster identity
    we no longer know which pollster contributed what to the daily estimate
    this makes it impossible to estimate house effects from this aggregated data (done in other file)

    combined sample size:
    we store the sum of all same-day sample sizes as n_polls_combined and use it for reporting only
    the combined variance (v*) is already computed directly via inverse-variance weighting and does not need n_combined for the kalman calculations, it is stored in sampling_var directly

    what about question-level variation within a poll?:
    in kalman_polling_bias.py, some polls contribute multiple rows (one per question wording)
    these are not independent since they share the same sample 
    here, all rows on the same day are collapsed together regardless of whether they come from the same poll or different polls
    this means multiple questions from the same poll are treated as if they were independent, which slightly overstates the effective sample size on days where one large poll dominates
    this is a known limitation and acceptabl given that the main motivation for aggregation is cross-pollster variation, not within-poll question variation
    """
    n_before = len(df)
    n_days_before = df['end_date'].nunique()

    # inverse-variance weighting:
    # weight for each poll = 1 / sampling_var (larger samples get more weight)
    df = df.copy()
    df['iv_weight'] = 1.0 / df['sampling_var']

    # weighted sum of margins and sum of weights, grouped by day
    day_groups = df.groupby('end_date')

    agg = day_groups.apply(
        lambda g: pd.Series({
            # inverse-variance weighted average margin
            'poll_margin':   np.average(g['poll_margin'], weights=g['iv_weight']),
            # combined variance = 1 / sum(weights)
            # in plain english: more polls = smaller combined uncertainty
            'sampling_var':  1.0 / g['iv_weight'].sum(),
            # combined sample size = sum of all sample sizes (for reporting only)
            'n_polls_combined': g['sample_size'].sum(),
            # number of individual polls combined into this day's estimate
            'n_questions_combined': len(g),
            # true margin is constant across all rows so just take first value
            'true_margin':   g['true_margin'].iloc[0],
        })
    ).reset_index()

    # sort chronologically
    agg = agg.sort_values('end_date').reset_index(drop=True)

    n_after = len(agg)
    print(f"\nwithin-day aggregation complete:")
    print(f"  observations before: {n_before} (individual poll questions)")
    print(f"  observations after:  {n_after} (one per unique end_date)")
    print(f"  unique days:         {n_days_before} -> {agg['end_date'].nunique()}")
    print(f"  mean polls per day:  {n_before / n_days_before:.1f}")
    print(f"  mean combined n:     {agg['n_polls_combined'].mean():.0f}")
    print(f"  mean combined var:   {agg['sampling_var'].mean():.4f} pp^2")
    print(f"  mean individual var (pre-agg): {df['sampling_var'].mean():.4f} pp^2")
    print(f"  variance reduction factor: {df['sampling_var'].mean() / agg['sampling_var'].mean():.1f}x")
...
def append_election_result(df: pd.DataFrame, election_date: str = '2024-11-05', anchor: bool = True) -> pd.DataFrame:
    ...
    anchor_row = {
        ...
        'sampling_var':          1e-6,         # effectively zero — marks this as the anchor
        'n_polls_combined':      0,             # not a real poll day
        'n_questions_combined':  0,
    }
...
def summarize_bias(df: pd.DataFrame, state: str, anchored: bool = True) -> pd.DataFrame:
    """
    print a decomposition of polling error into systematic vs random components.
    excludes the synthetic election result anchor row (identified by near-zero sampling_var)
    """
    # after aggregation there is no pollster column, so exclude anchor by sampling_var
    results = df[df['sampling_var'] > 1e-5].copy()
    true_margin = results['true_margin'].iloc[0]

    print("\n" + "=" * 60)
    if anchored:
        print(f"polling bias decomposition — {state} (trump margin, anchored)")
    else:
        print(f"polling opinion trajectory — {state} (trump margin, unanchored)")
    print("=" * 60)
    ...
    # variance decomposition: what share of total poll error variance is systematic (survives smoothing) vs random (filtered out)?
...
######################### Visualization ################################################
########################################################################################
def plot_results(df: pd.DataFrame, state: str, anchored: bool = True, save_path: str = None):
    """
    ...
    panel 3: systematic bias trajectory over time (anchored) or smoothed vs true (unanchored)
    """
    # after aggregation there is no pollster column, so exclude anchor by sampling_var
    results = df[df['sampling_var'] > 1e-5].copy()
    true_margin = results['true_margin'].iloc[0]
    ...
    fig.suptitle(
        f'Kalman Filtering & Smoothing: {state} 2024 ({mode_label})\n(Trump margin, pp | n={n_polls} daily aggregates)',
    ...
    ax1.scatter(dates, results['poll_margin'],
                ...
                label='aggregated daily polls', zorder=1)
    ...
    ax1.set_title('aggregated poll margins, filtered & smoothed estimates vs. true result', fontsize=11)
    ...
    ax2.plot(dates, conv_se,
             ...
             label='conventional se (aggregated)')
    ...
    # panel 3: systematic bias trajectory (anchored) or smoothed gap from true (unanchored)
    # positive values = polls overstated trump relative to the true result
    # negative values = polls overstated harris
    # in anchored mode: this is aggregate industry bias over time
    ...
    ax3.plot(dates, results['systematic_bias'],
             ...
             label='systematic bias (smoothed - true)' if anchored else 'smoothed - true')
...
def export_results(df: pd.DataFrame, out_path: str = 'kalman_results.csv') -> pd.DataFrame:
    ...
    note: after within-day aggregation, question_id/poll_id/pollster columns
    no longer exist (they were collapsed). we export the aggregated columns instead.
    """
    # base columns always present after aggregation
    cols = [
        'end_date',
        'poll_margin', 'true_margin',
        'n_polls_combined', 'n_questions_combined',
    ...
    # exclude the synthetic election result row (it has no pollster column after
    # aggregation, so we identify it by the near-zero sampling_var we set)
    out = df[df['sampling_var'] > 1e-5][cols].copy()
...
if __name__ == '__main__':
    ...
    log_filename = f'output/kalman/kalman_daily_aggregated_swingstates_log_{timestamp}.txt'
    logger = Logger(log_filename)
    sys.stdout = logger
    
    print(f"kalman filtering & smoothing analysis — swing states (within-day aggregated version)")
    ...
    for state in SWING_STATES:
        ...
        for days_before in TIME_WINDOWS:
            ...
            print("\n" + "="*70)
            print(f"ANCHORED MODE: bias decomposition conditional on known outcome ({state} | {window_desc})")
            print("="*70)
            
            df_anchored = load_and_prepare(DATA_PATH, state, ELECTION_DATE, days_before=days_before)
            print(f"\nsanity check — unique true_margin values: {df_anchored['true_margin'].unique()}")
            
            # aggregate same-day polls before anchoring and filtering
            df_anchored = aggregate_by_day(df_anchored)
            ...
            summarize_bias(df_anchored, state, anchored=True)
            plot_results(df_anchored, state, anchored=True, 
                        save_path=f'figures/kalman_agg_{state_abbrev}_bias_anchored_{window_label}.png')
            export_results(df_anchored, 
                          out_path=f'data/kalman_agg_{state_abbrev}_results_anchored_{window_label}.csv')

            # unanchored version: real-time opinion trajectory
            print("\n" + "="*70)
            print(f"UNANCHORED MODE: real-time opinion trajectory from polls alone ({state} | {window_desc})")
            print("="*70)
            
            df_unanchored = load_and_prepare(DATA_PATH, state, ELECTION_DATE, days_before=days_before)
            
            # aggregate same-day polls before anchoring and filtering
            df_unanchored = aggregate_by_day(df_unanchored)
            ...
            summarize_bias(df_unanchored, state, anchored=False)
            plot_results(df_unanchored, state, anchored=False, 
                        save_path=f'figures/kalman_agg_{state_abbrev}_trajectory_unanchored_{window_label}.png')
            export_results(df_unanchored, 
                          out_path=f'data/kalman_agg_{state_abbrev}_results_unanchored_{window_label}.csv')

            # comparison
            print("\n" + "="*70)
            print(f"COMPARISON: anchored vs unanchored ({state} | {window_desc})")
            print("="*70)
            ...
            final_smoothed_anchored   = df_anchored[df_anchored['sampling_var'] > 1e-5]['smoothed'].iloc[-1]
