...
log_file = open('output/fiftyplusone_analysis_datelimited_clustertwoway_with_partisan_and_lv_log_no_explode.txt', 'w')
sys.stdout = log_file
...
print("WITH MODE INDICATORS (NO EXPLOSION)")
...
# Get all unique mode types by splitting mode strings
all_modes = set()
...
# Create binary indicator for each mode type
for mode in sorted(all_modes):
    var_name = f'mode_{mode.replace(" ", "_").replace("-", "_")}'
    reg_df_original[var_name] = \
        reg_df_original['mode'].str.contains(mode, case=False, na=False, regex=False).astype(int)
    
    # Count how many polls use this mode
    n_with_mode = reg_df_original[var_name].sum()
...
# Set Live Phone as reference category (drop it from regression)
reference_mode = 'Live Phone'
reference_mode_var = 'mode_Live_Phone'

if reference_mode_var in mode_vars:
    mode_vars.remove(reference_mode_var)
    print(f"\nReference category: {reference_mode}")
    print(f"Dropped from regression: {reference_mode_var}")

print(f"\nMode indicators to include in regression: {mode_vars}")
print(f"All coefficients will be interpreted relative to polls using {reference_mode}")

# Show example of multi-mode encoding
print("\n" + "-"*110)
print("EXAMPLE: How multi-mode polls are encoded")
print("-"*110)

# Find a multi-mode poll example
multi_mode_example = reg_df_original[reg_df_original['mode'].str.contains('/', na=False)].head(3)
if len(multi_mode_example) > 0:
    for idx, row in multi_mode_example.iterrows():
        print(f"\nOriginal mode string: {row['mode']}")
        active_modes = [col.replace('mode_', '') for col in mode_vars + [reference_mode_var] 
                       if col in reg_df_original.columns and reg_df_original.loc[idx, col] == 1]
        print(f"Active indicators: {', '.join(active_modes)}")

# Create separate datasets by poll level (NO EXPLOSION - same sample size)
...
print(f"\n" + "="*110)
print("SAMPLE SIZES (MODE INDICATORS - NO EXPLOSION)")
print("="*110)
...
print("\nSwing States - Most Common Mode Combinations:")
mode_combos = reg_state_swing_original.groupby('mode').size().sort_values(ascending=False)
print(f"{'Mode String':<50} {'N':>10} {'%':>10}")
print("-" * 70)
for mode_str, count in mode_combos.head(15).items():
    pct = 100 * count / len(reg_state_swing_original)
    print(f"{mode_str:<50} {count:>10} {pct:>9.1f}%")

print(f"\nTotal unique mode combinations: {reg_state_swing_original['mode'].nunique()}")

# Count single-mode vs multi-mode polls
single_mode = (~reg_state_swing_original['mode'].str.contains('/', na=False)).sum()
multi_mode = (reg_state_swing_original['mode'].str.contains('/', na=False)).sum()
print(f"\nSingle-mode polls: {single_mode} ({100*single_mode/len(reg_state_swing_original):.1f}%)")
print(f"Multi-mode polls: {multi_mode} ({100*multi_mode/len(reg_state_swing_original):.1f}%)")

# Save if want to do VIF
reg_df_original.to_csv('data/harris_trump_regression_with_mode_indicators_and_lv_no_explode.csv', index=False)
...
# with mode (using indicators instead of exploded dummies)
...
swing_vars_ordered = ['duration_days', 'days_before_election', 'partisan_flag', 'pct_dk', 'abs_margin', 'turnout_pct'] + pop_vars
...
print(f"\n{'Mode':<20} {'National':>15} {'All States':>15} {'Swing':>15}")
print("." * 70)
...
# State variables
print("\nSwing state-level polls:")
corr_state_swing = reg_state_swing_original[state_x_vars_with_mode].corr()
print(corr_state_swing.to_string())

# State variables
print("\nState-level polls:")
corr_state = reg_state_original[state_x_vars_with_mode].corr()
print(corr_state.to_string())

# National variables
print("\n\nNational polls:")
corr_national = reg_national_original[national_x_vars_with_mode].corr()
...
create_correlation_heatmap(
    ...
    'figures/correlation_swing_no_mode_no_explosion.png',
    figsize=(14, 12)
)
...
create_correlation_heatmap(
    ...
    'figures/correlation_all_states_no_mode_no_explosion.png',
    figsize=(14, 12)
)
...
create_correlation_heatmap(
    ...
    'figures/correlation_national_no_mode_no_explosion.png',
    figsize=(14, 12)
)
...
corr_state_swing_mode = reg_state_swing_original[state_x_vars_with_mode].corr()
corr_state_mode = reg_state_original[state_x_vars_with_mode].corr()
corr_national_mode = reg_national_original[national_x_vars_with_mode].corr()
...
create_correlation_heatmap(
    ...
    'figures/correlation_swing_with_mode_no_explosion.png',
    figsize=(14, 12)
)
...
create_correlation_heatmap(
    ...
    'figures/correlation_all_states_with_mode_no_explosion.png',
    figsize=(14, 12)
)
...
create_correlation_heatmap(
    ...
    'figures/correlation_national_with_mode_no_explosion.png',
    figsize=(14, 12)
)
...
print("Accuracy analysis complete — see output/fiftyplusone_analysis_datelimited_clustertwoway_with_partisan_and_lv_log_no_explode.txt")